...
| C scrutiny | dp, gaffer, sound, composer, editor, motion-designer, colorist, character-continuity, **toon-translator**, **continuity-supervisor** | parallel read-only |
...
4. style-supervisor checks [cartoon-look-foundation.md](cartoon-look-foundation.md) anti-gloss list
...
```json
{ "skipPromptEnhancement": true, "stylePreset": "unstyled" }
```

Every `generation_prompt` must follow [cartoon-translation-foundation.md](cartoon-translation-foundation.md) — **toon-translator** scrutinizes after director merge.

**No `studio_generate_video` until Phase C signed off** with cartoon look on every shot **and** `storyboard_prompt` on every shot with cast on camera. Use `stylePreset: "unstyled"` + `skipPromptEnhancement: true` on all image/video gen ([direct-prompt-handoff.md](direct-prompt-handoff.md)).
