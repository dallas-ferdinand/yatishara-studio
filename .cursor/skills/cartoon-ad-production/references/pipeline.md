...
**Generation handoff:** All `studio_generate_image` / `studio_generate_video` use direct prompts — [direct-prompt-handoff.md](direct-prompt-handoff.md) (`unstyled` + `skipPromptEnhancement: true`; no Flash rewrite).

## Overview

Cartoon ad production mimics real film workflow: parallel department builds → director merge → cross-department scrutiny → iterate (max 3 rounds per phase).
...
| C | Shotcraft | editor (first), dp, gaffer, sound-designer, composer, motion-designer, colorist | director | Phase C builders + character-continuity + **toon-translator** + **continuity-supervisor** | `shot_packets[]` (+ `storyboard_prompt` when cast on camera) |
...
| Command | Stops at |
|---------|----------|
| `@cartoon-ad-production plan` | Budget approval |
| `@cartoon-ad-production run {slug}` | Final summary only |
