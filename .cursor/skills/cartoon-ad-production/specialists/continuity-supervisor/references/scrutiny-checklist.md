...
## Phase E — clip spatial integrity (Layer 5)

Canon: [../../../references/research-rounds/01-consistency-root-causes.md](../../../references/research-rounds/01-consistency-root-causes.md) §Spatial

- [ ] No wall line bend or room geometry warp during camera move
- [ ] Prop count and scale stable (two mugs stay two mugs)
- [ ] Witness object geography matches prior shot + world_packet
- [ ] Window/practical light from same camera-relative side throughout clip scrub
- [ ] No axis flip mid-clip unless motivated cut

## Phase E — start frame match

- [ ] Clip frame 1 matches E.5 still blocking (MIDGROUND position)
- [ ] FG elements in clip match storyboard FG — none invented at video step
...
Spatial issues logged here; prompt-format issues → **toon-translator**. Gates warn if `iteration_log.C` lacks `continuity-supervisor` entry ([gate-validation.md](../../../references/gate-validation.md)).
