...
| G-C proof | `iteration_log.C` has editor shot list **first**, `shot-reference-allocation.json`, parallel Phase C specialist builds (dp, gaffer, sound, composer, motion-designer, colorist), director merge, scrutiny from **all** Phase C builders + **seedance-translator** + **continuity-supervisor** |
...
| G-C translation | `seedance-translator` scrutiny pass with zero blocking; prompts per [seedance-translation-foundation.md](seedance-translation-foundation.md) |
...
9. Omitting [seedance-translation-foundation.md](seedance-translation-foundation.md) structure from shot prompts (vague prose, >100 words, full look prefix on I2V)
...
| C round N | `editor` shot list **first** → **parallel** dp, gaffer, sound, composer, motion-designer, colorist → director merge → **parallel** scrutiny (builders + seedance-translator + continuity-supervisor + character-continuity) | `generation/iterations/C-r{N}-*.json` |
