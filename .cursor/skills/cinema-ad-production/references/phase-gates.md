...
Orchestrator **must not** call any `studio_generate_image`, `studio_generate_element_sheet`, `studio_generate_video`, or `studio_generate_script` until **every gate below passes**.
...
| G-A proof | `iteration_log.A` has ≥1 round with `story-architect` build + `director-*` merge + scrutiny from `production-designer` + `character-continuity` |
...
| G-D seedance | Each scrutiny includes `seedance_checks` (grain, anti-gloss, motivated light) — not image sharpness alone |
...
| G-C proof | `iteration_log.C` has editor shot list **first**, `shot-reference-allocation.json`, parallel Phase C specialist builds (dp, gaffer, sound, composer, motion-designer, colorist), director merge, scrutiny from **all** Phase C roles |
...
9. Omitting [seedance-cinematic-look.md](seedance-cinematic-look.md) prefix from shot prompts
...
| C round N | `editor` shot list **first** → **parallel** dp, gaffer, sound, composer, editor, motion-designer, colorist → director merge → **parallel** scrutiny | `generation/iterations/C-r{N}-*.json` |
...
1. Load or create `{project}/generation/production-state.json`
...
4. Print gate status table before first `studio_generate_*` in each phase
