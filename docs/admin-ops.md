...
8. Manage **Academy** courses (Admin → Academy: course overview + free intro Stream video + multi-lesson CRUD with per-lesson banner/video, publish, grant/revoke access)
...
| Academy | [`AdminAcademyPane.tsx`](../src/studio/components/AdminAcademyPane.tsx) | Course: `academy.adminListCourses`, `adminUpsertCourse`, `adminSetCourseStatus`, intro via `academyActions.adminCreateStreamUpload` + cover `adminCommitCourseCover`; Lessons: `adminListLessons`, `adminUpsertLesson`, `adminSetLessonStatus`, `adminDeleteLesson`, `adminCreateLessonStreamUpload`, `adminCommitLessonCover`; purchases `adminListCoursePurchases` / grant / revoke |
