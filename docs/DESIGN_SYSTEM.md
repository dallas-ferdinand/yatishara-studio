...
- Feed watch caption Follow is a 28px idle bordered pill on the **right** of the name/time stack — not accent/`is-primary`, not a plus on the avatar. Boost and Contribute sit in that same right cluster (not in the bar below). Time under the name uses the same `ago · date` stamp as comments. On **mobile**, the owner’s own post uses a 28px icon-only Pencil in that cluster; desktop keeps the comments-dock pencil.
