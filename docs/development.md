...
- `ELEVENLABS_API_KEY` — Studio-only ElevenLabs key (voiceover `eleven_v3`, SFX, music `music_v2` with composition plans / extend / stems); Convex secrets + `.env.local`. Keep `ELEVENLABS_API_KEYS` as a single-entry array matching that key. Do not sync MercuryOS multi-account pool keys here. Shared voice library is on (`ELEVENLABS_LIBRARY_VOICES_ENABLED` defaults true; set `false` only to lock premade).
...
- `BUNNY_STREAM_TOKEN_AUTH_KEY` — optional; Stream library Token Authentication key for signed embeds (Academy intro + lesson playback). Intro playback is free (published course); lesson playback is entitlement-gated. Without the token key, embed URLs still respect those gates but are not token-signed.
