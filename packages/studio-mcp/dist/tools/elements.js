...
function registerElementTools(server) {
  ...
  server.tool(
    ...
    { elementId: z.string() },
    async ({ elementId }) => jsonResult(await studioFetch(`/elements/${encodeURIComponent(elementId)}`))
  );
  server.tool(
    ...
    CREATE_ELEMENT_GUIDE,
    {
      ...
      sheetAssetId: z.string().optional().describe("Existing uploaded image to use as the built sheet (for externally generated sheets)"),
      styleRules: z.string().optional().describe("Style Sheet only \u2014 markdown palette / line rules / forbidden drift"),
      renderMode: z.enum(["photoreal", "illustrated_2d", "illustrated_3d", "mixed"]).optional().describe("Style Sheet only")
    },
    async ({
      type,
      name,
  ...
  server.tool(
    ...
    UPDATE_ELEMENT_GUIDE,
    {
      ...
      renderMode: z.enum(["photoreal", "illustrated_2d", "illustrated_3d", "mixed"]).optional().describe("Style Sheet render mode")
    },
    async ({
      elementId,
      name,
