...
const estimateSchema = {
    ...
    videoModel: z
        .string()
        .optional()
        .describe('Video model slug. Default seedance-2.0. MCP-only: "kling-3.0-i2v", "veo-3.1" (faces via personGeneration allow_all).'),
};
const cinemaPresetHint = 'Cinema default: stylePreset "story-ad" with skipPromptEnhancement: true — observational look without preset rewrite of user prompt.';
export function registerGenerationTools(server) {
    ...
    server.tool("studio_list_video_models", "List video models for MCP selection. Includes MCP-only models (kling-3.0-i2v, veo-3.1) not shown in Studio UI.", {}, async () => jsonResult(await studioFetch("/video-models?scope=mcp")));
    ...
    server.tool("studio_generate_image", `Generate an image and save it to a Studio folder. Call studio_estimate_generation first. Uses wait=true (usually completes in seconds). ${cinemaPresetHint}`, {
        prompt: z.string(),
...
videoModel: default seedance-2.0 (Studio UI). MCP-only: kling-3.0-i2v (start-frame faces), veo-3.1 (Google Veo 3.1 I2V, personGeneration allow_all for face tests).`, {
        ...
        videoModel: z
            .string()
            .optional()
            .describe('Default seedance-2.0. MCP-only: "kling-3.0-i2v", "veo-3.1" (faces / real-person filter bypass tests).'),
    }, async (args) => {
    ...
    server.tool("studio_generate_script", `Generate a script document in a folder. Call studio_estimate_generation with mode=script first. Upload audio refs for voice briefs — Flash listens to attached audio. Returns documentId and title. ${cinemaPresetHint}`, {
        prompt: z.string(),
        folderId: z.string().optional(),
        stylePreset: z.string().optional(),
        referenceAssetIds: z.array(z.string()).optional(),
