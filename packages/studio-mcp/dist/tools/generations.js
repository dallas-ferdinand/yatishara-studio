...
export function registerGenerationTools(server) {
    ...
    server.tool("studio_generate_video", "Generate a video and save it to a Studio folder. Call studio_estimate_generation first. Starts async and polls until done (up to 5 min). Pass folderId or use key default.", {
        prompt: z.string(),
        folderId: z.string().optional(),
    ...
    server.tool("studio_generate_script", "Generate a script document in a folder. Call studio_estimate_generation with mode=script first. Upload audio refs for voice briefs — Flash listens to attached audio. Returns documentId and title.", {
        prompt: z.string(),
        folderId: z.string().optional(),
        stylePreset: z.string().optional(),
        referenceAssetIds: z.array(z.string()).optional(),
