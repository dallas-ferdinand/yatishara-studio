...
function registerContextTools(server) {
  server.tool(
    ...
    "[preferred] Start here. One-shot: account/credits + workspace tree + agent start-here hints. Optionally pass folderId or path for project_context. Replaces separate health + tree + BFS list_folders.",
    {
      ...
      path: z.string().optional().describe("Resolve path (and optionally ensure) for project context"),
      ensurePath: z.boolean().optional().describe("If path set and missing, create folders via ensure-path (needs write)"),
    ...
    async (args) => {
      ...
      if (args.path?.trim()) {
        ...
        try {
          ...
          folderId = resolved.folderId ?? folderId;
        } catch (error) {
  ...
  server.tool(
    ...
    "[preferred] Create nested folders for a path in one call (e.g. Clients/JAV/refs). Case-insensitive reuse of existing segments. Prefer over repeated studio_create_folder. Requires write scope.",
    {
      ...
      rootFolderId: z.string().optional().describe("Start folder; omit = sandbox root"),
      compact: z.boolean().optional()
    },
    async ({ path, rootFolderId, compact }) => jsonResult(
  ...
  server.tool(
    ...
    {
      query: z.string(),
      kinds: z.array(z.enum(["folder", "asset", "document", "element"])).optional(),
  ...
  server.tool(
    ...
    async ({ targetFolderId, items, compact }) => jsonResult(
