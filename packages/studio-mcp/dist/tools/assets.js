...
export function registerAssetTools(server) {
    ...
    server.tool("studio_upload_asset", "Upload a reference photo or media file to a folder. For element sheets: upload multiple angles first (see studio_element_sheet_guide). Requires write scope.", {
    ...
    server.tool("studio_update_asset", "Rename an asset (image, video, audio) or move it to another folder. Requires write scope.", {
        assetId: z.string(),
        name: z.string().optional(),
        folderId: z.string().optional(),
    }, async ({ assetId, name, folderId }) => jsonResult(await studioFetch(`/assets/${encodeURIComponent(assetId)}`, {
        method: "PATCH",
