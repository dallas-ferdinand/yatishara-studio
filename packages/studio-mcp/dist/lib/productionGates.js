...
const SIGNED_OFF = /* @__PURE__ */ new Set(["signed_off_clean", "signed_off_with_compromises"]);
function signoffOk(state, phase) {
  const status = state.phase_signoffs?.[phase]?.status;
  return status !== void 0 && SIGNED_OFF.has(status);
}
...
function findShot(state, shotId) {
  if (!shotId) return void 0;
  return state.shot_packets?.find((s) => s.shot_id === shotId);
}
function cartoonPrefixPresent(prompt) {
  if (!prompt?.trim()) return false;
  return prompt.includes("Preserve start-frame cartoon look") || prompt.includes("flat cel shading") || prompt.includes("consistent line weight") || prompt.includes("PRESERVE start-frame");
}
const PHOTOREAL_LEAK_RE = /\b(Shot on ARRI Alexa|film grain|natural skin texture|documentary-grade|photorealistic|live-action footage|observational camera)\b/i;
const FORBIDDEN_ZOOM_RE = /\b(zoom\s+in|zoom\s+out|snap\s+zoom|optical\s+zoom)\b/i;
const FULL_LOOK_ON_I2V_RE = /\b(Traditional 2D cel animation|flat two-tone shading|Shot on ARRI Alexa)\b/i;
const TRAVEL_VERB_RE = /\b(dolly|pan|track|push-in|pull-out|crane|orbit)\b/i;
function validateToonTranslation(prompt, opts) {
  const issues = [];
...
const ECU_CU_FACE_RE = /\b(ECU|extreme close[- ]?up|close[- ]?up|CU)\b.*\b(face|eyes|portrait|head[- ]?on)\b/i;
function validateStoryboardPrompt(prompt) {
  ...
  if (!prompt.includes("flat cel") && !prompt.includes("2D cel") && !prompt.includes("stylized") && !prompt.includes("cartoon")) {
...
function validateConsistencyConstraints(prompt) {
  const issues = [];
  if (!prompt?.trim()) return issues;
  if (!prompt.includes("CONSTRAINTS:")) {
    issues.push("missing CONSTRAINTS: header");
  } else if (!/no (wardrobe|ink|photoreal|color)/i.test(prompt) && !/preserve start/i.test(prompt)) {
    issues.push("CONSTRAINTS should include preserve + no drift clauses");
  }
  return issues;
}
function validateProductionGates(args) {
  ...
  if (["E5", "E", "GENERATE"].includes(target)) {
    ...
    for (const shot of state.shot_packets ?? []) {
      ...
      const storyboardIssues = shot.cast_on_camera ? validateStoryboardPrompt(shot.storyboard_prompt) : [];
      ...
      if (shot.cast_on_camera && !cartoonPrefixPresent(shot.generation_prompt) && useBlockers) {
        blockers.push(
    ...
    const targetDuration = state.story_packet?.duration_sec ?? state.shot_packets?.reduce((s, sh) => s + (sh.duration_sec ?? 0), 0);
    const sceneCount = Array.isArray(state.story_packet?.scenes) ? state.story_packet.scenes.length : void 0;
