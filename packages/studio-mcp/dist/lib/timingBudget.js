...
export function resolveDurationTier(durationSec) {
    for (const tier of DURATION_TIERS) {
        if (durationSec >= tier.minSec && durationSec <= tier.maxSec)
            return tier;
    }
    return null;
}
export function validateTimingBudget(args) {
    ...
    if (Math.abs(editorialSum - target) > DURATION_TOLERANCE_SEC) {
        issues.push(`shot durations sum ${editorialSum}s ≠ target ${target}s (allowed ±${DURATION_TOLERANCE_SEC}s)`);
    }
    const genSum = shots.reduce((sum, s) => sum + (s.generation_duration_sec ?? 0), 0);
    if (genSum < editorialSum - DURATION_TOLERANCE_SEC) {
        issues.push(`generation_duration_sec sum ${genSum}s < editorial ${editorialSum}s — trim budget impossible`);
    }
    const tier = resolveDurationTier(target);
    if (tier) {
        if (shots.length < tier.minShots || shots.length > tier.maxShots) {
            issues.push(`shot count ${shots.length} outside ${tier.label} budget ${tier.minShots}–${tier.maxShots}`);
        }
        const scenes = args.sceneCount;
        if (scenes !== undefined && (scenes < tier.minScenes || scenes > tier.maxScenes)) {
            issues.push(`scene count ${scenes} outside ${tier.label} budget ${tier.minScenes}–${tier.maxScenes}`);
        }
    }
    else {
    ...
    for (const shot of shots) {
        ...
        if (gen !== undefined && gen > 0 && gen < SEEDANCE_MIN_GEN_SEC) {
            issues.push(`shot ${shot.shot_id ?? "?"} generation_duration_sec ${gen}s below Seedance minimum ${SEEDANCE_MIN_GEN_SEC}s`);
        }
        const editorial = shot.duration_sec ?? 0;
        if (editorial > 0 && editorial < 1.5) {
            issues.push(`shot ${shot.shot_id ?? "?"} editorial duration ${editorial}s too short — min 1.5s for readable beat`);
        }
    }
    return issues;
}
