...
export function registerDocumentTools(server: McpServer) {
  server.tool(
    ...
    { documentId: z.string() },
    async ({ documentId }) =>
      jsonResult(await studioFetch(`/documents/${encodeURIComponent(documentId)}`)),
  );
  ...
  server.tool(
    ...
    async ({ folderId, title, contentMarkdown }) =>
      jsonResult(
  ...
  server.tool(
    ...
    async ({ documentId, title, contentMarkdown, folderId }) =>
      jsonResult(
