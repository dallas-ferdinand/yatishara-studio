...
export function registerFolderTools(server: McpServer) {
  ...
  server.tool(
    ...
    async ({ folderId, compact }) =>
      jsonResult(await studioFetch(`/folders/${encodeURIComponent(folderId)}`), compact),
  );
  ...
  server.tool(
    ...
    async ({ name, parentId, icon, color, compact }) =>
      jsonResult(
  ...
  server.tool(
    ...
    "Rename a folder or move it under another parent. Requires write scope. Cannot move the API key sandbox root folder.",
    {
      ...
      parentId: z
        .string()
        .optional()
        .describe("New parent folder ID (use sandbox root to move to top level)"),
      compact: z.boolean().optional(),
    },
    async ({ folderId, name, icon, color, parentId, compact }) =>
      jsonResult(
