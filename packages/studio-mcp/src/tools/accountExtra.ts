...
/**
 * Wave 4 MCP tools — billing depth + notifications.
 *
 * Intended HTTP paths (handlers in convex/studioApiAccountExtra.ts; mount later):
 *   GET  /account/payments              (scope: read)
 *   GET  /account/payments/:id          (scope: read)
 *   GET  /account/credits               (scope: read)
 *   GET  /account/plans                 (scope: read)
 *   GET  /account/pricing               (scope: read)
 *   GET  /account/storage               (scope: read)
 *   GET  /account/subscription          (scope: read) — optional extras
 *   GET  /notifications                 (scope: social)
 *   POST /notifications/:id/read        (scope: social)
 *
 * Existing studio_health / studio_credit_balance still use GET /account.
 */
export function registerAccountExtraTools(server: McpServer) {
  server.tool(
  ...
  server.tool(
    ...
    { paymentId: z.string() },
    async ({ paymentId }) =>
      jsonResult(await studioFetch(`/account/payments/${encodeURIComponent(paymentId)}`)),
  );
