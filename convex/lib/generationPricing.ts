 ...
 *
 * Text (`google/gemini-2.5-flash-lite`): $0.10/M input (text/image/video), $0.30/M audio input,
 * $0.40/M output — priced from estimated tokens × 2.
 */
...
/** @deprecated Text uses 2× Flash Lite COGS — see textCreditCost. */
export const PLATFORM_OVERHEAD_CREDITS_TEXT = 0;

/**
 * Gemini 2.5 Flash-Lite (GATEWAY_TEXT_MODEL_ID) — USD per million tokens.
 * Text / image / video input share the $0.10 rate; audio input is $0.30.
 */
export const TEXT_USD_PER_M_INPUT = 0.1;
export const TEXT_USD_PER_M_OUTPUT = 0.4;
export const TEXT_USD_PER_M_AUDIO_INPUT = 0.3;

/** Typical script / element-notes job without media refs. */
export const TEXT_BASE_INPUT_TOKENS = 6_000;
export const TEXT_BASE_OUTPUT_TOKENS = 2_500;

/** Approximate multimodal input tokens added per reference. */
export const TEXT_IMAGE_REF_INPUT_TOKENS = 1_500;
...
export const TEXT_GENERATION_BASE_CREDITS = 1;
...
/** Customer TT$ for script / element text = 2× model COGS, rounded up to TT$0.50. */
export function textSellPriceTtd(args: {
  imageReferenceCount?: number;
  videoReferenceCount?: number;
  audioReferenceCount?: number;
}): number {
  return roundUpToHalfTtd(estimateTextModelUsd(args) * USD_TO_TTD * 2);
}
