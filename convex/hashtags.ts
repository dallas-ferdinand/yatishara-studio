...
async function suggestHashtagsImpl(
  ctx: QueryCtx,
...
export const suggestHashtags = query({
  ...
  handler: async (ctx, args) => {
    return await suggestHashtagsImpl(ctx, args);
  },
});
...
async function suggestPeopleImpl(
  ctx: QueryCtx,
  args: { query: string; limit?: number; expiresUnix?: number },
) {
  const limit = Math.min(Math.max(args.limit ?? 8, 1), 20);
...
export const suggestPeople = query({
  ...
  handler: async (ctx, args) => {
    return await suggestPeopleImpl(ctx, args);
  },
});

export const suggestHashtagsForApi = internalQuery({
  args: {
    userId: v.id("users"),
    query: v.string(),
    limit: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    ...
    return await suggestHashtagsImpl(ctx, {
      query: args.query,
      limit: args.limit,
    });
  },
...
export const suggestPeopleForApi = internalQuery({
  args: {
    userId: v.id("users"),
    query: v.string(),
    limit: v.optional(v.number()),
  ...
  handler: async (ctx, args) => {
    const user = await ctx.db.get("users", args.userId);
    if (!user) throw new Error("User not found");
    return await suggestPeopleImpl(ctx, {
      query: args.query,
      limit: args.limit,
