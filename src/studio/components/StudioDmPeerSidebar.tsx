...
type PeerTab = "notes" | "jobs" | "labels" | "about" | "shared";
...
/** Landing / History / Files-style bottom sheet: grab handle, peek↔full, flick dismiss. */
function PeerMobileSheet({
  onClose,
  children,
}: {
  onClose: () => void;
  ...
  const refreshMetrics = () => {
    ...
    const peek = readTokenPx(
      root,
      "--studio-mobile-dm-peer-sheet-height",
      window.innerHeight * 0.48,
    );
    const full = readTokenPx(
      root,
      "--studio-mobile-dm-peer-sheet-full",
  ...
  const settleTo = (fromH: number, target: number) => {
    ...
    window.requestAnimationFrame(() => {
      applyHeight(target);
      setIsFull(
        target >= full - 8 || target > peek + (full - peek) * 0.5,
      );
  ...
  return (
    <div
      ref={sheetRef}
      className={`studio-mobile-app-menu-sheet studio-dm-peer-mobile-sheet${entered ? " is-entered" : " is-entering"}${isFull ? " is-full" : ""}${dragging ? " is-dragging" : ""}${settling ? " is-settling" : ""}`}
      role="dialog"
      aria-modal="true"
    ...
    >
      ...
      <div className="studio-dm-peer-mobile-sheet-body">{children}</div>
    </div>
  );
}
...
export function StudioDmPeerSidebar({
  ...
  const sharedWithPeer = useQuery(
    api.studioShares.listOutgoingToPeer,
    open ? { peerUserId, expiresUnix } : "skip",
  );
  ...
  const revokeShare = useMutation(api.studioShares.revokeShare);
  ...
  const panelNode = (
    <aside
      ...
      aria-label="Chat details"
    >
      <div className="cursor-panel-head cursor-sidebar-head shrink-0">
        {tabNav}
      </div>
      <div
  ...
  if (variant === "sheet" || isMobile) {
    ...
    return createPortal(
      <PeerMobileSheet onClose={onClose}>
