...
const DEFAULT_SCHEME_ID = DEFAULT_WALLPAPER.themeId;
...
function resolveSchemeForWallpaper(wallpaper, cached) {
  const fallbackId =
    wallpaper?.kind === "preset" && SCHEMES[wallpaper.themeId]
      ? wallpaper.themeId
      : DEFAULT_SCHEME_ID;
  const base = SCHEMES[fallbackId] ?? SCHEMES[DEFAULT_SCHEME_ID] ?? SCHEMES.agent;
...
function applyDeskTokens(palette, isLight) {
  ...
  root.style.setProperty("--color-cursor-text-bright", palette.text);
...
function migrateLegacy() {
  ...
  if (id === "light" && !localStorage.getItem(MODE_KEY)) {
    ...
    localStorage.setItem(SCHEME_KEY, DEFAULT_SCHEME_ID);
  }
...
export function getSchemeId() {
  ...
  const id = localStorage.getItem(SCHEME_KEY);
  if (id === "light") return DEFAULT_SCHEME_ID;
  return SCHEMES[id] ? id : DEFAULT_SCHEME_ID;
}
...
export function setColorScheme(id) {
  ...
  void setWallpaper({
    kind: "preset",
    family,
    themeId: SCHEMES[id] ? id : DEFAULT_SCHEME_ID,
  });
}
  ...
  const wallpaper =
    ref?.kind === "asset" && ref.assetId
      ...
      : {
          ...
          themeId: SCHEMES[ref?.themeId] ? ref.themeId : DEFAULT_SCHEME_ID,
        };
