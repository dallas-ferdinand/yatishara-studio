...
export type WaveformProps = HTMLAttributes<HTMLDivElement> & {
  ...
  /** Peak bar height as a fraction of the canvas (default 0.8). */
  amplitude?: number
  active?: boolean
...
export const Waveform = ({
  ...
  useEffect(() => {
    ...
    const renderWaveform = () => {
      ...
      const peakScale = Math.max(0.1, Math.min(1, amplitude))

      for (let i = 0; i < barCount; i++) {
        const dataIndex = Math.floor((i / barCount) * data.length)
        const value = data[dataIndex] || 0
        const barHeight = Math.max(baseBarHeight, value * rect.height * peakScale)
...
export const ScrollingWaveform = ({
  ...
  const nextBarHeight = (index: number) => {
    if (data && data.length > 0) {
      return data[index % data.length] || 0.1
    }
    const seededRandom = (i: number) => {
      const x = Math.sin(seedRef.current * 10000 + i * 137.5) * 10000
      return x - Math.floor(x)
    }
    const wave1 = Math.sin(index * 0.1) * 0.2
    const wave2 = Math.cos(index * 0.05) * 0.15
    const randomComponent = seededRandom(index) * 0.4
    return Math.max(0.1, Math.min(0.9, 0.3 + wave1 + wave2 + randomComponent))
  }
  ...
  useEffect(() => {
    ...
    resizeObserver.observe(container)
    return () => resizeObserver.disconnect()
    // eslint-disable-next-line react-hooks/exhaustive-deps -- seed once per mount/size props
  }, [barWidth, barGap, data])
    ...
    const animate = (currentTime: number) => {
      ...
      if (speed > 0) {
        ...
        while (
          ...
          const nextX = lastBar ? lastBar.x + step : 0
          const index = dataIndexRef.current
          dataIndexRef.current = index + 1
          barsRef.current.push({
            x: nextX,
            height: nextBarHeight(index),
          })
...
export const AudioScrubber = ({
  ...
  const [fallbackWaveform] = useState(() =>
    Array.from({ length: 100 }, () => 0.2 + Math.random() * 0.6),
  )
  const waveformData = data.length > 0 ? data : fallbackWaveform
