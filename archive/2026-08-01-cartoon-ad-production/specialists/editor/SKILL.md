...
2. [../../references/micro-pacing-foundation.md](../../references/micro-pacing-foundation.md) — **shot rhythm within scenes (legato/staccato patterns)**
3. [../../references/perceptual-foundation.md](../../references/perceptual-foundation.md) §6–7
...
8. [references/scene-rhythm-patterns.md](references/scene-rhythm-patterns.md)
9. [references/cut-types.md](references/cut-types.md)
...
0. **`scene_rhythms[]`** per [../../references/micro-pacing-foundation.md](../../references/micro-pacing-foundation.md) — `pattern_id`, per-shot `duration_sec`, `role_in_scene`, `perceived_pace_register` **before** shot-level camera_intent
1. Shot list: `shot_id`, `scene_id`, `duration_sec`, `generation_duration_sec`, `editorial_trim_sec`
2. **`rhythm` block** on each shot — `pattern_id`, `role_in_scene`, `perceived_pace_register`, `cut_cue`
3. **`camera_intent`** per shot — beat, energy, move family, size arc, layer device, rhythm, contrast note
...
- Multi-shot scene missing `pattern_id` or `scene_rhythms[]` entry (blocking)
- Adjacent scenes: legato vs staccato without `pace_shift_intent` (blocking)
- `opener_ratio` ≥0.55 labeled staccato (blocking)
- Editorial ≤1.5s shot paired with long travel in merged CAMERA prose (blocking w/ toon-translator)
