...
```json
{
  ...
  "research_refs": ["research:kuleshov-sequence", "research:peak-end-rule", "research:murch-rhythm"]
}
```
...
- `blocking`: adjacent scenes different `perceived_pace_register` without `pace_shift_intent` ([micro-pacing-foundation.md](micro-pacing-foundation.md))
- `blocking`: multi-shot scene missing `pattern_id` in `scene_rhythms[]`
