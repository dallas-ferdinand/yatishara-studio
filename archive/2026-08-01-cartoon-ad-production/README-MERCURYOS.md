...
| `studio_list_presets` | Style presets (includes `unstyled` for direct handoff) |
...
For `@cartoon-ad-production` automated runs — **direct handoff, no Flash/GPT rewrite**:

```json
{ "stylePreset": "unstyled", "skipPromptEnhancement": true }
```

See `.cursor/skills/cartoon-ad-production/references/direct-prompt-handoff.md`. Pass `stylePresetSlug: style_family` on `studio_generate_element_sheet` only.

`raw` aliases `unstyled`.
