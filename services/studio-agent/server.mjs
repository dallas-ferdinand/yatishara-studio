...
async function runPiTurn(body, abortSignal) {
  ...
  try {
    ...
    const system = [
      ...
      "Prompt save: when they ask for a NEW prompt or script (write/craft/create from scratch) — skills first, then studio_create_document into CWD with NON-EMPTY contentMarkdown. Body must be plain markdown only: title + optional ```text fence + ## References with `- [Label](asset://{assetId})` links. NEVER pipe-meta rows (`| kind: image | studio: id`) — those crash Script open. Never create an empty Script. After create, keep the returned documentId. Never stash the prompt body in remember/memory. Title like \"Prompt — <short>\" or \"Script — <short>\". Chat: only paste the prompt if they asked to see/copy it; otherwise point at the file.",
